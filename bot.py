...
def grab_accounts(US, GB, DE, CA, AU, IT, NZ, MX, key, BE, FR, ID, SG, BR, MY, PT, IL, DK, NL, ES, SE, NO, TR, TH, HU):
    ...
    f.close()
    f = open('Accounts/TH.txt', 'r')
    for line in f:
        clean = line.split('\n')
        TH.append(clean[0])
		
    f.close()
    f = open('Accounts/HU.txt', 'r')
    for line in f:
        clean = line.split('\n')
        HU.append(clean[0])
...
async def redeem(ctx, arg1, arg2, arg3):
    ...
    allowed_countries = [
     'US', 'GB', 'DE', 'CA', 'AU', 'IT', 'NZ', 'MX', 'BE', 'FR', 'ID', 'SG', 'BR', 'MY', 'PT', 'IL', 'DK', 'NL', 'ES', 'SE', 'NO', 'TR', 'TH', 'HU']
    accounts = []
    keys = []
    country = arg1.upper()
    keyGrab(keys)
...
async def stock():
    ...
    HU_stock = []
    key = []
    grab_accounts(US_stock, TH_stock, HU_stock, GB_stock, DE_stock, CA_stock, AU_stock, IT_stock, NZ_stock, MX_stock, key, BE_stock, FR_stock, ID_stock, SG_stock, BR_stock, MY_stock, PT_stock, IL_stock, DK_stock, NL_stock, ES_stock, SE_stock, NO_stock, TR_stock)
    embed = discord.Embed(title='Stock',
      colour=discord.Colour.blue())
    embed.set_author(name='Actual Stock [Skrrr]', icon_url='https://cdn.discordapp.com/attachments/349859530516987904/561596862063181870/tobaccosmoke_1440613.png')
    ...
    embed.add_field(name='TH', value=len(TR_stock), inline=True)
    embed.add_field(name='HU', value=len(TR_stock), inline=True)
